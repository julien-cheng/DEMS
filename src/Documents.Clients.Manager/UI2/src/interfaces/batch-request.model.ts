...
export class BatchRequest {
  operationsBatch?: IBatchRequestOperations;
  options: IRequestOptions = {
    headers: { 'Content-type': 'application/json' }, //headers: HttpHeaders = new HttpHeaders({ 'Content-type': 'application/json' });
    apiUrl: 'api/batch',
    responseType: 'json',
  };
  ...
  eventType?: EventType;
  rows?: ItemQueryType[];
  observableRequest?: Promise<IBatchResponse>;
  ...
  requestDialogConfirmation: { required: boolean; message?: string; isSendDisabled?: boolean } = {
    required: false,
  };
      ...
      this.rows.map(row => {
        !!row.allowedOperations &&
          row.allowedOperations.forEach(operation => {
            operation.batchOperation.type === this.requestType &&
              this.batchOperations.push(operation.batchOperation);
          });
      });
    }
  ...
  private setRequireDialogConfirmation(requestType: BatchRequestType) {
    ...
    this.requestDialogConfirmation.required = !!(batchOperationsDefaults.dialogMessages as any)[
      requestType
    ];
    if (!!(batchOperationsDefaults.dialogMessages as any)[requestType]) {
      let msg = (batchOperationsDefaults.dialogMessages as any)[requestType],
        typeLabel = 'group of items';
      // !!this.rows && this.rows.length < 2
      //   ? new FileTypePipe().transform(this.rows[0].type)
      //   : 'group of items';
  ...
  public setHeader(type?: string) {
    this.options.headers = type ? { 'Content-type': type } : { 'Content-type': 'application/json' };
    return this.options.headers;
  }

  // Set misc request header
  public setMoreHeader(headers: object) {
