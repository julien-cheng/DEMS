...
export type FilterFunction = { name: string, errorMessage: string, fn: (file?: IUploadedFile, options?: IFileUploadOptions) => boolean };
...
export class FileUpload {
    ...
    public isQueueUploading: boolean = false; // Labels the current state of the fileUpload (globally)
    public queue: Array<FileObject> = [];
    public failedQueue: Array<FileObject> = [];
    public progress: number = 0;
    public isIE11: boolean = false;
    ...
    protected _setIsIE11Flag() {
        let msie = parseInt((/trident\/.*; rv:(\d+)/.exec(navigator.userAgent.toLowerCase()) || [])[1]); // parseInt((/msie (\d+)/.exec(navigator.userAgent.toLowerCase()) || [])[1]);
    ...
    protected _setFileUploadOptions(options: IFileUploadOptions): void {
        ...
        if (this.options.maxFileSize) {
            let max = this.fileSizePipe.transform(this.options.maxFileSize, true);
    ...
    public _initAutoUpload() {
        ...
        this.options.autoUpload && this._uploadAllFiles();
    }
    ...
    public _fileisReadyForUpload(fileObject: FileObject): boolean {
        return (!fileObject.fileObjectStatus.isUploading && !fileObject.fileObjectStatus.isUploaded && !fileObject.fileObjectStatus.isPaused && fileObject.fileObjectStatus.isReady);
    }
    ...
    public _setPausedUploadedFiles(): Array<any> {
        return this.queue.filter((fileObject: FileObject, index: number) => {
            (index >= this.options.concurrentUploadLimit) && fileObject.setFileObjectStatus({ isPaused: true });
        });
    }

    // Description:  Unpause next item in queue
    public _nextReadyForUploadFile(): FileObject {
        let nextFO = this.queue.find((fileObject) => {
            return !fileObject.fileObjectStatus.isUploaded && fileObject.fileObjectStatus.isPaused;
        });
        (nextFO) && nextFO.setFileObjectStatus({ isReady: true, isPaused: false });
        return nextFO;
    }
    ...
    public onBeforeItemsAddtoQueue(files: IUploadedFile[], fullPath?: string): IUploadedFile[] {
        let webkitRelativeFullPath, _fullpath = '';
        for (let i = 0; i < files.length; i++) {
            _fullpath = fullPath;
            if (fullPath === null) { // This is an input upload and fullpath is null
                webkitRelativeFullPath = files[i].webkitRelativePath;
                _fullpath = '/' + (!!webkitRelativeFullPath && webkitRelativeFullPath.length ? webkitRelativeFullPath : files[i].name);
            }
            files[i].fullPathName = _fullpath;
        };
        return files;
    }
    ...
    public _onSuccessCallback(fileObject: FileObject, response: any, status: number, headers: HttpHeaders): void {
        ...
        (this.options.removeAfterUpload) && this.removeFromUploadQueue(fileObject);
    ...
    public _onErrorCallback(fileObject: FileObject, response: any, status: number, headers: HttpHeaders): void {
        ...
        (fileObject.options.retries >= this.options.maxNumberRetries) &&
            this.addToFailedQueue(fileObject);
    }
    ...
    public _onCompleteCallback(fileObject: FileObject, response: any, status: number, headers: HttpHeaders): void {
        ...
        this.progress = this._getTotalProgress();
        this.fireEvent({ eventName: FILEUPLOAD_EVENTS.complete, fileObject: fileObject});

        //Reinitialize upload if more items remain
        if (this._getPausedUploadedFiles().length > 0) {
            const next = this._nextReadyForUploadFile();
            (this.options.autoUpload || next.options.autoUpload) && this.uploadFile(next);
        }
    }

    public getReadyItems(): Array<any> {
        return this.queue
            .filter((fileObject: FileObject) => (fileObject.fileObjectStatus.isReady && !fileObject.fileObjectStatus.isUploading));
    }
    ...
    public processFileforUpload(fileObject: FileObject): void {
        const transport = this.options.isHTML5 && fileObject.options.isChunkedUpload ? '_chunkedUpload' :
            this.options.isHTML5 && !fileObject.options.isChunkedUpload ? '_xhrTransport' : '_iframeTransport';
    ...
    protected _xhrTransport(fileObject: FileObject): any {
        const _self = this,
            ...
            url = options.url;
        _self._initRequestData(file, options);
        const requestOptions = {
            ...
            responseType: <any>options.dataType,
        ...
        fileObject._uploadProcess = this.http.request(req)
            .subscribe((event) => {
                switch (event.type) {
                    case HttpEventType.Sent:
                        _self._onProgressFileObject(fileObject, 0);
                        break;
                    case HttpEventType.DownloadProgress:
                    case HttpEventType.UploadProgress:
                        if (event.total) {
                            let progress = Math.round((event.loaded / event.total) * 100);
                            (progress === 100) && (progress = 98); // Let the final 100 be set by the actual respons
                            _self._onProgressFileObject(fileObject, progress);
                        }
                        break;
                    case HttpEventType.Response:
                        _self._onSuccessCallback(fileObject, event.body, event.status, event.headers);
                        break;
                }
            },
            (error) => {  // Handle errors:
                _self._onErrorCallback(fileObject, error.body, error.status, error.headers);
            });
    ...
    protected _chunkedUpload(fileObject: FileObject) {
        ...
        let fileSize = file.size,
            ...
            url = options.url,
            uploadedBytes = options.uploadedBytes, // start
            maxChunkSize = options.maxChunkSize || fileSize, // end
            isMultiChunkedFile= fileSize >= options.maxChunkSize, //Local to store muliple blob uploads
            numberOfChunks = Math.max(Math.ceil(fileSize / maxChunkSize), 1),
            end;
        ...
        function upload() {
            ...
            let requestOptions: any = {
                headers: _self._initRequestHeader(options.headers, file, options),
                params: _self._initRequestParams(options),
                reportProgress: options.reportProgress || false,
                responseType: <any>options.dataType,
                withCredentials: options.withCredentials || false
            },
            ...
            fileObject._uploadProcess = _self.http.request(req)
                .subscribe((event) => {
                    switch (event.type) {
                        case HttpEventType.Sent:
                            break;
                        ...
                        case HttpEventType.Response:
                            ...
                            uploadedBytes = _self._getUploadedBytes(headers) || (uploadedBytes + options.chunkSize);
                            ...
                            let token = !!data.files && data.files[0].token ? data.files[0].token :
                                !!data.response.token ? data.response.token : !!data.token ? data.token : null;

                            url = options.url + '&token=' + encodeURIComponent(token);
                            (uploadedBytes < fileSize) ? upload() :
                                _self._onSuccessCallback(fileObject, data, event.status, headers);
                            break;
                    }
                },
                (error) => {
                    console.error('POST call in error', error);
                    if (!_disableRetries && (_self.options.maxNumberRetries > fileObject.options.retries)) {
    ...
    protected _initRequestData(file: File, options: IFileObjectOptions): IFileObjectOptions {
        let _self = this, formData, inData,
            _blob = (options.isChunkedUpload) ? options.blob : file;
    ...
    protected _initRequestParams(options: IFileObjectOptions): HttpParams {
        ...
        if (options.additionalParameter !== undefined) {
            Object.keys(options.additionalParameter).forEach((key: string) => {
                let paramValue = options.additionalParameter[key]
    ...
    _getUploadedBytes(responseHeaders: HttpHeaders) {
        let range = responseHeaders.get('Range'),
