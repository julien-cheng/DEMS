...
export class FileObject {
    ...
    public _increaseRetry(): number { // console.log('_increaseRetry: ', this.options.retries);
    ...
    protected _setFileObjectOptions(options: IFileUploadOptions, Ifile: IUploadedFile): void {
        this.options = Object.assign({
            ...
            retries: 0,
            additionalParameter: {
                fileInformation: { // Add all file params to the fileInformation object
                    lastModified: Ifile.lastModified,
                    lastModifiedDate: Ifile.lastModifiedDate,
                    name: Ifile.name,
                    size: Ifile.size,
                    type: Ifile.type,
                    fullPath: this._extractFullPath(Ifile.fullPathName) || '',
                    webkitRelativePath: Ifile.webkitRelativePath
                }
            }
        }, options);
    }
    ...
    protected _extractFullPath(fullPathName: string): string {
        let index = fullPathName.lastIndexOf('\/'),
            fullPath = index > 0 ? fullPathName.substr(0, index) : '/';
        return fullPath;
    }
