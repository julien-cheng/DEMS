...
import { IFileIdentifier, IMediaSet, IMediaSource, IMediaSubtitles, IMediaSegment } from '../index';
import { HttpClient, HttpHeaders } from '@angular/common/http';
import { Observable } from 'rxjs/Observable';
import { BatchRequest, IBatchResponse } from '../index';
...
export class MediaToolsService {
  ...
  buildMediaSources(sources: any[]): IMediaSource[] {
    let safeSources = [], fileUrl;
    sources.forEach((source) => {
      fileUrl = this.getFileContentURL(source.fileIdentifier);
      let obj = Object.assign(source, {
        saferesourceURL: this.sanitizer.bypassSecurityTrustResourceUrl(fileUrl)
      });
  ...
  public setActiveSegment(segments: IMediaSegment[], activeSegment: IMediaSegment): IMediaSegment[] {
    return (!!activeSegment) ? segments.map((segment) => {
      segment['isActive'] = _.isEqual(segment, activeSegment) || ((segment.startTime === activeSegment.startTime) && (segment.endTime === activeSegment.endTime) && (segment.text === activeSegment.text));
      return segment;
    }) : segments;
  }
  ...
  public resetActiveSegment(segments: IMediaSegment[]): IMediaSegment[] {
    return !!segments && segments.filter((segment) => {
  ...
  public getSegmentIndexByStartTime(segments: IMediaSegment[], currentTime: number, isMsec: boolean = false): number {
    let segmentIndex: number, i = 0;
    !isMsec && (currentTime = currentTime * 1000); // Convert to msec if its not assed as such
    (!!segments && !Number.isNaN(currentTime)) && segments.every((segment: IMediaSegment) => {
  ...
  public getSegmentIndexByTime(segments: IMediaSegment[], startTime: number, endTime: number): number {
    let segmentIndex: number = -1,
     i = 0;
    !!segments && segments.every((segment: IMediaSegment) => {
  ...
  public saveMediaSegments(mediaSet: IMediaSet): Observable<IBatchResponse> {
    ...
    if (!!mediaSet.rootFileIdentifier.organizationKey && !!mediaSet.rootFileIdentifier.folderKey && !!mediaSet.rootFileIdentifier.fileKey) {
      return this.http.put<IBatchResponse>(url, mediaSet, options)
  ...
  public addNewSegmentByStartTime(segments: IMediaSegment[], segment: IMediaSegment): IMediaSegment[] {
    if (!!segments) {
      const start = segment.startTime;
      let index: number = 0;
      (segments.length > 0) && segments.forEach((s, i) => { // console.log(start, s.startTime,i);
  ...
  public getSegmentIndex(segments: IMediaSegment[], segment: IMediaSegment): number {
    if (!!segments && segment) {
      return segments.findIndex(searchSegment => {
        return _.isEqual(searchSegment, segment) || ((searchSegment.startTime === segment.startTime) && (searchSegment.endTime === segment.endTime) && (searchSegment.text === segment.text));
      });
    } else {
      return -1;
    }
