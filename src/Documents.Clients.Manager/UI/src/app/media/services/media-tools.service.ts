...
export class MediaToolsService {
  ...
  buildMediaSubtitles(subtitle: any[]): IMediaSubtitles[] {
    ...
    subtitle.forEach(subtitle => {
      ...
      const obj = Object.assign(subtitle, {
        saferesourceURL: this.sanitizer.bypassSecurityTrustResourceUrl(fileUrl)
      });
  ...
  public getFileContentURL(fileIdentifier: IFileIdentifier) {
    return `/api/file/contents?fileidentifier.organizationKey=${fileIdentifier.organizationKey}&fileidentifier.folderKey=${
      fileIdentifier.folderKey
    }&fileidentifier.fileKey=${fileIdentifier.fileKey}&open=true`;
  }
  ...
  public defaultNullableMuteSegment(segments: IMediaSegment[]): IMediaSegment[] {
    return !!segments
      ? segments.map(segment => {
          if (!Array.isArray(segment.mutes)) {
            segment.mutes = [];
          }
          return segment;
        })
  ...
  public getSegmentIndexByStartTime(segments: IMediaSegment[], currentTime: number, isMsec: boolean = false): number {
    ...
    } // Convert to msec if its not assed as such
    !!segments &&
  ...
  public saveMediaSegments(mediaSet: IMediaSet): Observable<IBatchResponse> {
    ...
    if (!!mediaSet.rootFileIdentifier.organizationKey && !!mediaSet.rootFileIdentifier.folderKey && !!mediaSet.rootFileIdentifier.fileKey) {
      return this.http.put<IBatchResponse>(url, mediaSet, options).pipe(catchError(this.handleError));
    } else {
