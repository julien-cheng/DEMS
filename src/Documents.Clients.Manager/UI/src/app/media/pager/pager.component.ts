...
export class PagerComponent implements IMediaPagination, OnChanges {
  @Output() onPageChanged = new EventEmitter<number>();
  @Input() public pageNumber: number;
  @Input() public pageSize: number; // How many items per page
  @Input() public items: any[]; // SearchResult Items array
  pageIndex: number; // pageNumber - 1
  pagerCount = 5; // Number of pager links per segments = between the << and >>
  ...
  ngOnChanges(simpleChanges: SimpleChanges) {
    ...
    if (!!simpleChanges.pageNumber) {
      this.pageIndex = Number(this.pageNumber - 1);
    }
    this._buildPagerObj();
  }
