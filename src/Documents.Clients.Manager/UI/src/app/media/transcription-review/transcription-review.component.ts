import { Component, OnInit, Input, ViewChild, Inject, Output, EventEmitter, SimpleChanges } from '@angular/core';
...
export class TranscriptionReviewComponent {
  ...
  ngOnChanges(simpleChanges: SimpleChanges) {
    (!!simpleChanges.activeSegment && !simpleChanges.activeSegment.firstChange  && !!this.activeSegment) && this.scrollTopForActiveSegment();
  }
  ...
  private scrollTopForActiveSegment(){
    ...
    if(!this.isEditing 
      ...
      let offsetTop = document.getElementById(`${this.activeSegment.startTime}`).offsetTop;
      let elementHeight = document.getElementById(`${this.activeSegment.startTime}`).clientHeight;
      let scrollTo = offsetTop - elementHeight - 40;
  ...
  public putMediaSegments(turnEditModeOff: boolean) {
    ...
    this.mediaToolsService.saveMediaSegments({
      rootFileIdentifier: this.fileIdentifier,
      segments: this.segments
    }).subscribe(
      (response: IBatchResponse) => {
        const mediaSet = <IMediaSet>response.response;
