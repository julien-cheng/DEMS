...
export class MediaSearchComponent implements OnInit {
  ...
  @Input() enableShareLink:boolean =false;
  ...
  public openShareModal(event: Event, segment: IMediaSegment){
    let destination = `/${this.route.snapshot.url.join('/')}?startTime=${segment.startTime}&endTime=${segment.endTime}`;
    
    this.shareLinkSegment = segment;
    
    this.shareLinkSegmentURL= !!this.canonicalURLFormat ? this.canonicalURLFormat.replace('__ENCODED_PATH__', encodeURIComponent(destination)).replace('__PATH__', destination):
      `${window.location.origin}/${destination}`;
   
    this.$('#shareSegment').modal() .on('hidden.bs.modal', (e) => {
