...
export class BatchOperationService {
  ...
  destroyBatchOperationOnComplete = true; // set to true on complete of the request type cycle - default to no intermediate step

  constructor(private toastr: ToastrService, protected http: HttpClient) {
  ...
  public setBatchOperationEvents(): any {
    const self = this;
    let filter: (batchRequest: BatchRequest) => boolean; // BatchRequestFn;
    const actionObj = {};
    BatchRequestTypes.forEach((batchRequestType: BatchRequestType) => {
      const eventObj = {};
      EventTypes.forEach((eventType: EventType) => {
        filter = null;
        const fnName = batchRequestType + '_' + eventType;
        filter = !!(this as any)[fnName]
          ? (this as any)[fnName]
          : eventType === 'initialize'
          ? self.BaseRequest_initialize
          : eventType === 'send'
          ? self.BaseRequest_send
          : eventType === 'success'
          ? self.BaseRequest_success
          : eventType === 'error'
          ? self.BaseRequest_error
          : eventType === 'complete'
          ? self.BaseRequest_complete
          : null;
        if (!!filter) {
          eventObj[eventType] = filter;
        }
      });
  ...
  public processBatchRequestAction(requestBatchData: IRequestBatchData) {
    ...
    if (!!this.batchRequestFns[this.batchRequest.requestType]) {
      const response =
        !!this.batchRequestFns[this.batchRequest.requestType][this.batchRequest.eventType] &&
      ...
      const response =
        !!this.batchRequestFns['BaseRequest'][this.batchRequest.eventType] &&
  ...
  private BaseRequest_initialize(batchRequest: BatchRequest) {
    ...
    if (batchRequest.requestDialogConfirmation.required) {
      // Most delete/remove actions will go through here: "DeleteRequest", "RemoveRecipientRequest"
  ...
  private BaseRequest_send(batchRequest: BatchRequest): Observable<any> {
    ...
    if (!!batchRequest.operationsBatch) {
      // console.log("batchRequest.operationsBatch is not null");
  ...
  private DownloadRequest_initialize(batchRequest: BatchRequest): boolean {
    ...
    batchRequest.operationsBatch.operations.map(operation => {
      fileURL = `/api/file/contents?fileidentifier.organizationKey=${operation.fileIdentifier.organizationKey}&fileidentifier.folderKey=${
        operation.fileIdentifier.folderKey
      }&fileidentifier.fileKey=${operation.fileIdentifier.fileKey}`;
    });
    this.response.success = true; // HARD CODED //(!!operation.folderKey && !!operation.fileKey);

    this.response.success ? (window.location.href = fileURL) : console.error('The file download url is malformed');
    this.batchRequest.observableRequest = of(this.response);
    return true;
  }
  ...
  private DownloadZipFileRequest_initialize(batchRequest: BatchRequest) {
    ...
    this.batchResponse.loadBatchResponse({
      ...
      callbackFunctionArg: {
        operations: batchRequest.operationsBatch.operations
      },
      displayMessageOnSuccess: true,
      triggerSuccessEventOnSuccess: false
    });
    return of(this.response); // this.batchRequest.observableRequest = Observable.of(this.response);
  }
  ...
  private MoveIntoRequest_initialize(batchRequest: BatchRequest) {
    ...
    return (this.batchRequest.observableRequest = of(this.response));
  }
  ...
  private AddRecipientRequest_send(batchRequest: BatchRequest): Observable<any> {
    ...
    const options = batchRequest.options,
      headers = new HttpHeaders({ 'Content-type': 'application/json' }),
      requestOptions = { headers }, // new RequestOptions({ headers }),
      body = {
        FirstName: options.params.get('newRecipientFName'),
        LastName: options.params.get('newRecipientLName'),
        RecipientEmail: options.params.get('newRecipientEmail'),
        FolderIdentifier: options.params.get('folderIdentifier'),
        type: 'AddRecipientRequest'
      };
    const arg = Object.assign(this.batchResponse, this.batchResponse);
    this.batchResponse.loadBatchResponse({
  ...
  private DisplayRecipientRequest_initialize(batchRequest: BatchRequest) {
    ...
    const row = batchRequest.rows[0] as IRecipient;
    return (this.batchRequest.observableRequest = of({
      success: true,
      response: {
        name: row.name,
        email: row.email,
  ...
  isPathMoveIntoItem(itemQueryType: ItemQueryType, moveintoType: 'sourceFile' | 'sourcePath' | 'targetPath'): boolean {
    ...
    if (!!itemQueryType.allowedOperations) {
      const key = moveintoType + 'Identifier';
      const allowedOperationsArr = this.retrieveRequestOperation([itemQueryType], 'MoveIntoRequest');
  ...
  public batchPost(batchRequest?: BatchRequest, extraHttpRequestParams?: any): Observable<IBatchResponse> {
    ...
    return this.batchPostWithHttpInfo(batchRequest, extraHttpRequestParams).pipe(
  ...
  public displayButtonByOperation(allowedOperation: IAllowedOperation): boolean {
    return (
      NoButtonRequestTypes.filter(function(el) {
