...
export class AuthService {
  ...
  loginUser(userName: string, password: string): Observable<IUser> {
    ...
    return this.http.post<IUser>('api/login', loginInfo, options).pipe(
      tap(response => {
        if (response) {
          this.currentUser = response as IUser;
        }
      })
    );
  }
  ...
  ediscoveryAuthenticateUser(formValues: { email: string; password: string; token: string }): Observable<IBatchResponse> {
    ...
    return this.http.post<IBatchResponse>('api/ediscovery/authenticateuser', loginInfo, options).pipe(
      tap((result: IBatchResponse) => {
        if (result) {
  ...
  leoAuthenticateUser(formValues: { email: string; password: string; token: string }): Observable<IBatchResponse> {
    ...
    return this.http.post<IBatchResponse>('api/leoupload/authenticateuser', loginInfo, options).pipe(
      tap((result: IBatchResponse) => {
        if (result) {
          return result.response;
        }
