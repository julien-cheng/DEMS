...
export class FileService {
    ...
    fileGet(fileIdentifier: IFileIdentifier, pathIdentifier: IPathIdentifier): Observable<IBatchResponse> {
        const url = '/api/file?organizationKey=' + fileIdentifier.organizationKey + '&folderKey=' + fileIdentifier.folderKey +
            '&pathKey=' + pathIdentifier.pathKey + '&fileKey=' + fileIdentifier.fileKey;
        if (!!fileIdentifier.organizationKey && !!fileIdentifier.folderKey && !!fileIdentifier.fileKey) {
            ...
            return Observable.throw(new Error('File error in fileGet'));
        }
    }
    ...
    getFileMediaSet(fileIdentifier: IFileIdentifier, viewertype: string): Observable<IBatchResponse> {
        ...
        if (!!fileIdentifier.organizationKey && !!fileIdentifier.folderKey && !!fileIdentifier.fileKey) {
            ...
            return Observable.throw(new Error('Error in Get File Media Set'));
        }
    }
    ...
    getDetailViewType(file: IFile): string {
        const detailViewType = (file && file.viewerType !== 'none' && file.viewerType !== 'Unknown') ? file.viewerType :
            (file && (file.viewerType === 'none' || file.viewerType === 'Unknown')) ? this.getFileTypefromExt(file) : 'default';
