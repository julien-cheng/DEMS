...
import { IManager, ViewMode, TIdentifierType, ItemQueryType, IPath, IPathTreeNode, IBreadcrumb, IPathIdentifier,IFileIdentifier } from '../index';
...
export class ExplorerService {
    ...
    buildBreadCrumbs(pathIdentifier: IPathIdentifier): IBreadcrumb[] {
        let bradcrumbArr = [];
        let pathtree = this.fileExplorer.currentExplorer.pathTree;
    ...
    getTree(branch, activeNode, arr) {
        if (!!branch.paths) {
            let subBranch = branch.paths.filter((path) => {
        ...
        const _self = this,
            arr = aPath.map((oPath) => {
                ...
                (arrayHolder.children !== undefined && arrayHolder.children.length) ?
                    arrayHolder.children.push(convertedObj) : arrayHolder.push(convertedObj);

                (oPath.paths !== undefined && oPath.paths !== null) &&
    ...
    fnBuildPathNodeObject(oPath: IPath, sufix: string = ''): IPathTreeNode {
        ...
        let isExpanded = oPath.isExpanded ? oPath.isExpanded :
