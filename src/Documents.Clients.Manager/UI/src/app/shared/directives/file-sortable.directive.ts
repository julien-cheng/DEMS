      ...
      $(this.el)
        .draggable({
          ...
          helper(event) {
            ...
            draggingRowItems.forEach(f => {
              f.uiClass = 'ui-dragging';
            });
    ...
    if (this.dropTarget) {
      $(this.el).droppable({
        ...
        over(event, ui) {
          ...
          $(event.target).closest('.node-wrapper').length
            ? $(event.target)
        ...
        out(event, ui) {
          ...
          $(event.target).closest('.node-wrapper').length
            ? $(event.target)
                .closest('.node-wrapper')
                .removeClass('ui-state-highlight not-dropabble')
        ...
        drop(event, ui) {
          ...
          if (!!sourceArr && sourceArr.length <= 1 && isProhibited) {
            ...
            if (!!identifier) {
              const batchoperations = _self.batchOperationService.retrieveRequestOperation(
                sourceArr,
                'MoveIntoRequest'
              ) as IBatchOperation[];
