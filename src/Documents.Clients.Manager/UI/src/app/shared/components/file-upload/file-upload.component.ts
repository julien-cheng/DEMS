...
export class FileUploadComponent implements OnChanges {
  ...
  constructor(public auth: AuthService, public fileUpload: FileUpload, private toastr: ToastrService) {}

  ngOnChanges(simpleChanges: SimpleChanges) {
    this.url =
      '/api/upload?pathIdentifier.organizationKey=' +
      this.pathIdentifier.organizationKey +
      '&pathIdentifier.folderKey=' +
      this.pathIdentifier.folderKey +
      (this.pathIdentifier.pathKey ? '&pathIdentifier.pathKey=' + encodeURIComponent(this.pathIdentifier.pathKey) : '');
  ...
  onCompleteCallback(event: any) {
    ...
    const msg = !!event.fileObject.file.name
      ? event.fileObject.file.name + ' was uploaded successfully!'
