...
export class FileExplorerComponent implements OnInit {
  @Input() isMoveAction = false; // Creates another node type without redirect actions
  ...
  constructor(private batchOperationService: BatchOperationService, public explorerService: ExplorerService) {}
  ...
  ngOnInit() {
    ...
    !this.currentTreeExplorer && (this.currentTreeExplorer = this.explorerService.fileExplorer.currentTreeExplorer);
  }
  ...
  onTreeUpdate() {
    ...
    !this.isMoveAction &&
      (!!this.currentTreeExplorer && this.currentTreeExplorer.length > 0) &&
  ...
  setCurrentIsExpanded(children: any) {
    ...
    !!children &&
      children.forEach(element => {
        const expanded = this.explorerService.getObjects(element, 'isExpanded', true);
  ...
  isDropTarget(itemQueryType: ItemQueryType): boolean {
    const t = this.batchOperationService.isPathMoveIntoItem(itemQueryType, 'targetPath');
  ...
  saveMoveItem(event: Event, target: TreeNode) {
    ...
    const rows: ItemQueryType[] = this.batchOperationService.batchRequest.rows,
      key = target.data.identifier;
    let batchoperations: IBatchOperation[] = [];
    batchoperations = this.batchOperationService.retrieveRequestOperation(rows, 'MoveIntoRequest') as IBatchOperation[];
  ...
  saveCallback(requestBatchData: IRequestBatchData, rows?: ItemQueryType[]) {
    const data = !!rows ? Object.assign(requestBatchData, { rows }) : requestBatchData;
