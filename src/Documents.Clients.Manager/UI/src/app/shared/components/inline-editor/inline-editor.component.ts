import { Component, Input, ElementRef, ViewChild, Renderer, forwardRef, OnInit, SimpleChanges, Output, EventEmitter } from '@angular/core';
...
export class InlineEditorComponent implements ControlValueAccessor, OnInit {
  ...
  private patternRegex: string = '^[a-zA-Z0-9-_. ]+'; // This may come from the server
  public maxLength: number = 250; // This may come from the server
  constructor(
    element: ElementRef,
  ...
  saveInlineEdit($event: Event) {
   if (this.newPathName.invalid) { // Field is required and value is empty - handle empty editors
      this._value = this.preValue;
      return this.cancelInlineEdit($event);
    };
    
    this.row.allowedOperations.forEach(
      (operation) => {
  ...
  cancelInlineEdit($event: Event){
    const _self = this;
    setTimeout(function(){ // (needs a setTimeout) otherwise it interferes with saves button clicks
      _self.editing = false;
      _self.row.editMode = false;
    }, 500);
  }
