...
import { IGridView, ItemQueryType, IAllowedOperation, IGridColumnSpecification, ViewMode, IGridViewProperties, IBatchOperation, BatchRequestType } from '../index';
...
export class GridViewModel {
    ...
    public selectedBatchOperations: {[batchRequestType: string]: IBatchOperation[]};
    ...
    private _setGridViewProperties(gridViewProperties?: IGridViewProperties, resetAllStatus: boolean = true) {
        if(resetAllStatus){
            let defaultOAbyRow = this.rows.filter(row=>{return !!row.allowedOperations && row.allowedOperations.length>0; }),
            propDefaults = {
                allSelected: false,
                selectedItem: null,
                selectedItems: [],
    ...
    public intersectRows(rows: ItemQueryType[]): IAllowedOperation[] {
        ...
        allowedoperations = intersectionBy(...arrOfAO, (e) => { return e.batchOperation.type && e.displayName });
        ...
        if (!!rows) {
            rows.map((row) => {
                row.allowedOperations.forEach(
                    (operation) => {
                        ((operation.batchOperation.type === requestType) && (operation.displayName === displayName)) && objectTyped.push(operation.batchOperation);
                    }
                );
            });
        }
    ...
    public getAllowedOperationsButtonDependent(operations: IAllowedOperation[]): IAllowedOperation[] {
        ...
        if (!!operations) {
            showBtnGroup = operations.filter((operation) => {
                let displayButton: boolean = (operation.batchOperation.type === 'MoveIntoRequest') ? // Do not add menu buttons for non-source 
                    (!!operation.batchOperation.sourceFileIdentifier || !!operation.batchOperation.sourcePathIdentifier) : ((operation.batchOperation.type !== 'UploadRequest') ? true : false);
