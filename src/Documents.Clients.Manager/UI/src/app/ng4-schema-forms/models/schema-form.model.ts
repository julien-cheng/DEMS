...
import { IBaseControlOptions, SCHEMAFORMS_EVENTS, IBaseSchema, Schema, IArraySchema, IBooleanSchema, INullSchema, IObjectSchema, IStringSchema, INumberSchema } from '../index';
...
export class SchemaForm {
        ...
        if (properties) {
            for (let key in properties) {
                if (properties.hasOwnProperty(key)) {
                    let initialValue = initialFormValue !== undefined ? initialFormValue[key] :
                        this.initialFormValue !== undefined ? this.initialFormValue[key] : undefined;
    ...
    public toFormGroup(properties: BaseControl[]) {
        ...
        properties.forEach((prop) => {
            ...
            group[prop.key] = (!!prop.validator.validatorArr && prop.validator.validatorArr.length) ? new FormControl(prop.value, Validators.compose(prop.validator.validatorArr))
