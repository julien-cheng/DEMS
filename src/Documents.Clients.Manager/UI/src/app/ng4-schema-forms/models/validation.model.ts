...
export class Validation {
  public isRequired = false; // for ui required label and instructions
  public isValidated = false; // if false => status= untouched or notvalidated / if true => status= valid or error
    ...
    if (!!schemaValidators && schemaValidators.length) {
      schemaValidators.forEach((validator: IValidator, i: number) => {
        ...
        if (!!Validators[validator.type]) {
          ...
          const newValFn: ValidatorFn =
            validator.type === 'required' || validator.type === 'requiredTrue' || validator.type === 'email'
          ...
          // else this is a custom validator:
          const addValidator = Object.assign({}, _.find(formCustomValidators, val => val.name === validator.value));
          this.customValidators.push(addValidator); // May not need this
          const nameKey = 'customValidator_' + i,
            valFn: ValidatorFn = control => {
              const result = addValidator.fn.call(this, control);
  ...
  public validateControl(value: any | undefined, schema: Schema, ngFormControl: AbstractControl) {
    ...
    for (const validatorKey in ngFormControl.errors) {
      if (ngFormControl.errors.hasOwnProperty(validatorKey)) {
        // console.log(ngFormControl.errors[validatorKey].name);
        const vvk: IValidatorValueKeys = ngFormControl.errors[validatorKey];
        // Adjust key to 'custom'
        const key = this._isCustomValidator(validatorKey) ? vvk.name : validatorKey;
  ...
  private _getValidatorsErrorMessages(key: string, value: IValidatorValueKeys, schema: Schema): string {
    ...
    const isCustom: boolean = value.type === 'custom';
    const valObj: IValidator = this._getValidator(key, isCustom);
    // Make Key = custom for errorMessage
    isCustom && (key = 'custom');
    if (!!valObj && !!valObj.errorMessage) {
      // Default to server side message (overrides default when populated)
      msg = valObj.errorMessage;
    } else if (!!DefaultSettings.error[key]) {
      // else
      ...
      switch (key.toLowerCase()) {
        ...
        case 'email':
          msg =
            type === 'boolean' && key === 'required'
              ? DefaultSettings.error.requiredTrue.replace('{0}', label)
  ...
  private _isSame(value1: ValueType, value2: ValueType) {
    ...
    for (const key in value1) {
      if (
        value1.hasOwnProperty(key) &&
        !this._isSame((value1 as { [name: string]: ValueType })[key], (value2 as { [name: string]: ValueType })[key])
      ) {
        return false;
      }
    }
