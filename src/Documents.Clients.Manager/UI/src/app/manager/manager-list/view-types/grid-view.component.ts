...
export class GridViewComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.pagination = this.viewItem as IPagination;
    this.galleryItems = this._getGalleryItems();
  }

  private _getGalleryItems() {
    const galleryItems = this.gridView.rows.filter(item => {
      if ((item as IFile).viewerType === 'Image') {
        const img = item as IFile;
        const url = `/api/file/contents?fileidentifier.organizationKey=${item.identifier.organizationKey}&fileidentifier.folderKey=${
          item.identifier.folderKey
        }&fileidentifier.fileKey=${item.identifier.fileKey}&open=true`;
        const thumbUrl = !!(item as IFile).previewImageIdentifier
          ? `/api/file/contents?fileidentifier.organizationKey=${img.previewImageIdentifier.organizationKey}&fileidentifier.folderKey=${
              img.previewImageIdentifier.folderKey
            }&fileidentifier.fileKey=${img.previewImageIdentifier.fileKey}&open=true`
  ...
  private _setRowUIProperties(rows: ItemQueryType[]) {
    if (rows.length > 0) {
      rows.map(row => {
        ...
        row.dropSource =
          this.batchOperationService.isPathMoveIntoItem(row, 'sourcePath') ||
        ...
        row.rowInlineValidators =
          row.type === 'ManagerPathModel'
            ? this.gridviewValidators.pathNameValidationPatterns
            : row.type === 'ManagerFileModel'
            ? this.gridviewValidators.fileNameValidationPatterns
            : [];
      });
    }
  }
  ...
  processMultipleBatchRequest(requestBatchData: IRequestBatchData, isMultiple: boolean = true) {
    if (isMultiple) {
      ...
      if (!!this.gridView.selectedBatchOperations[requestBatchData.requestType]) {
        Object.assign(requestBatchData, {
          rows: this.gridView.selectedRowItems,
          batchOperations: multiBatchOperations
        });
      }
    }
