    ...
    template: `
        <div class="list-view row">
            <div class="col-12 col-sm-6 col-md-3 mb-1" *ngFor="let row of rows; let i=index" >
                <div class="icon-box {{row.uiClass}}" 
                    [ngClass]="{'selected': row.selected, 'editMode':row.editMode}"
                    [appFileSortable]="row"
                    [dropTarget]="row.dropTarget"  [dropSource]="row.dropSource"  [gridView]="gridView"
                    ...
                    <div class="hover-toolbar">
                        <div class="pretty float-left" *ngIf="hasRowsAllowedOperations">
                            <input type="checkbox" (change)="changeSelectRow.emit({selected: $event.target.checked, row: row,index: i})"
                    ...
                    <div class="icon-box-content"> 
                        <p>
                            <app-link [row]="row" [imageLink]="true" (triggerCallback)="saveCallback($event)"></app-link>
                        </p>
                        <ng-container *ngIf="!row.editMode">
                            <app-link [row]="row" (triggerCallback)="saveCallback($event)" [hideIcon]="true"></app-link>
                        </ng-container>
                        <app-inline-editor [row]="row" [(ngModel)]="newName" label="{{row.name}}" [editing]="row.editMode" [required]="true" 
