import { Component, OnInit, AfterContentInit, Input, Output, HostListener, NgZone, EventEmitter, ElementRef, ViewChildren, QueryList, Renderer2 } from '@angular/core';
...
@Component({
    ...
    <div class="carousel-indicators-wrapper" [ngClass]="{'indicators-centered': !isScrollable}">
        <ol class="carousel-indicators" 
            [style.width]="getSafeStyle(indicatorWidth + 'px')" 
            [style.left]="thumbnailsLeft"  >
            <li *ngFor="let item of galleryItems;let i=index" [ngClass]="{'active': i===0}" 
                [attr.data-src]="item.safeThumbUrl" 
                [attr.data-slide-to]="i" 
                data-target="#gallery" [style.width]="getSafeStyle(_thumbWidth + 'px')"  #thumbSlide >
            </li>
        </ol>
    </div>
...
export class ImageGalleryThumbnailComponent implements OnInit {
    ...
    thumbWidth: number = 120; // This is the default width - will adjust to windows width 
    thumbMarginHorizontal: number = 6; // Includes the margin 3px + 120px  // thumbMarginHorizontal: number = 6;
    ...
    isPrevActive: boolean = false; // is the prev arrow active
    isNextActive: boolean = true; // is the next arrow active
    isScrollable: boolean = true; // The number of images and total width is greater than the viewport = is scrollable
    ...
    public visibleRangeIndexBottom: number = 0; // The thumb that is in the first position
    ...
    ngAfterViewInit() {
        this.thumbs.forEach((li) => { this.imageLiArr.push(li); }); // Build the array(
        this.lazyLoadImages();
    }
    ...
    moveSlides(action: string) {
        if (action === 'next') {
            ...
            this.visibleRangeIndexBottom--;
        } else { // Resets/shifts to  active slide in gallery or first slide 
            this.visibleRangeIndexBottom = ((this._activeSlideIndex + this._visibleCount) <= this.count) ? this._activeSlideIndex : (this.count - this._visibleCount);
        ...
        this.updateThumbSliderRange.emit({ 'bottom': this.visibleRangeIndexBottom, 'top': this.visibleRangeIndexTop });
    }

    // Description: Update the left position of the slider - transition
    setThumbnailsPosition(): void {
        this.zone.run(() => { // Needs zone for detecting changes in main carousel and syncing functionalities
    ...
    public lazyLoadImages() {
        if (!!this.imageLiArr && this.imageLiArr.length > 0) {
            ...
            const btm = this.visibleRangeIndexBottom,
                top = ((this.visibleRangeIndexTop + this._visibleCount) < this.galleryItems.length) ? (this.visibleRangeIndexTop + this._visibleCount) :
    ...
    public adjustThumbSliderOnSlid(activeIndex: number) {
        ...
        (activeIndex >= maxIndex || activeIndex < this.visibleRangeIndexBottom) && this.moveSlides('reset');
    }
}